...
public class CreateWarehouseGeneratorSnowflake extends AbstractSqlGenerator<CreateWarehouseStatement> {
    ...
    public ValidationErrors validate(CreateWarehouseStatement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ...
        CreateWarehouseStatement.ValidationResult result = statement.validate();
        ...
        // Special logic: Auto-enable query acceleration if scale factor is set but acceleration is not explicitly enabled
        if (statement.getQueryAccelerationMaxScaleFactor() != null && statement.getEnableQueryAcceleration() == null) {
