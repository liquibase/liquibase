...
class GenerateJsonChangeLogPostgresIntegrationTest extends Specification {
    @Shared
    ...
    def "Should generate JSON changelog incl. NULL-values"() {
        given:
...
""")
    ...
    def "Should generate JSON changelog excl. NULL-values"() {
        given:
        def changelogFileName = "target/test-classes/changelogs/pgsql/update.changelog.xml"
        def resourceAccessor = new SearchPathResourceAccessor(".,target/test-classes")
        def scopeSettings = [
