...
DECISION_PROTOCOL:
  ...
  IF_YES:
    ...
    OPTION_B: "Need to add new database-specific attributes"
      ...
      COMPLEXITY: "MEDIUM"
      EXAMPLES: ["createTable with transient", "createSequence with ORDER/NOORDER"]
      GOTO: "EXISTING_CHANGETYPE_EXTENSION"
      CRITICAL_ARCHITECTURAL_REQUIREMENT: "MUST use namespace attributes, NOT database-specific change classes"
...
## 🚨 CRITICAL: ARCHITECTURAL VIOLATION PREVENTION

### NAMESPACE_ATTRIBUTE_EXTENSION vs DATABASE_SPECIFIC_CHANGE_CLASS Decision Matrix

```yaml
ARCHITECTURAL_ENFORCEMENT:
  FOR_EXISTING_LIQUIBASE_CHANGETYPES:
    RULE: "MUST use namespace attribute extension pattern"
    FORBIDDEN: "Creating database-specific change classes like CreateSequenceChangeSnowflake"
    EXAMPLES:
      CORRECT_APPROACH: "Use namespace attributes on standard createSequence changetype"
      VIOLATION_EXAMPLE: "Creating CreateSequenceChangeSnowflake extends CreateSequenceChange"
    
  IMPLEMENTATION_REQUIREMENTS:
    NAMESPACE_PARSER: "MUST utilize existing SnowflakeNamespaceAwareXMLParser"
    NAMESPACE_STORAGE: "MUST utilize existing SnowflakeNamespaceAttributeStorage" 
    SQL_GENERATOR: "MUST extend standard generators, not create new change classes"
    XSD_SCHEMA: "MUST define namespace attributes, not new element types"
    
  AI_CONSUMPTION_VALIDATION:
    DETECTION_PATTERN: "If creating class ending with 'ChangeSnowflake' for existing changetype = VIOLATION"
    COMPLIANCE_CHECK: "Verify namespace parser supports the changetype in isTargetChangeType()"
    XSD_VALIDATION: "Ensure namespace attributes defined in XSD, not new elements"
```

### XSD Schema Requirements for Namespace Attributes

```yaml
XSD_INTEGRATION_REQUIREMENTS:
  MANDATORY_DURING_REQUIREMENTS_PHASE:
    - "Define namespace attributes in XSD schema"
    - "Validate attribute types and constraints" 
    - "Test namespace attribute parsing"
    - "Verify SQL generator integration"
    
  XSD_ATTRIBUTE_PATTERN:
    LOCATION: "src/main/resources/www.liquibase.org/xml/ns/snowflake/liquibase-snowflake-latest.xsd"
    SYNTAX: "<xsd:attribute name=\"orderSequence\" type=\"xsd:boolean\"/>"
    NAMESPACE: "http://www.liquibase.org/xml/ns/snowflake"
    
  PARSER_INTEGRATION:
    TARGET_CHANGETYPES: "Must be listed in SnowflakeNamespaceAwareXMLParser.isTargetChangeType()"
    ATTRIBUTE_EXTRACTION: "Must extract objectName in getObjectName() method"
    STORAGE_INTEGRATION: "Must store in SnowflakeNamespaceAttributeStorage"
```
