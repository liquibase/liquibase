...
### Step 1: Database Object Structure Discovery (XSD-Driven Approach)
```yaml
BLOCKING_VALIDATION_1.1:
  TYPE: "XSD_BASED_OBJECT_STRUCTURE_COMPLETE"
  REQUIREMENT: "Complete database object structure discovered using XSD as primary source (85% coverage) plus supplementary metadata discovery (15% coverage)"
  DELIVERABLE: "Complete object structure map with XSD attributes + state attributes, all properties mapped and validated"
  FAILURE_ACTION: "STOP - Must have complete XSD compliance + state attribute coverage before proceeding"

RESEARCH_ACTIVITIES:
  XSD_ATTRIBUTE_EXTRACTION:
    - "Parse liquibase-snowflake-latest.xsd to extract ALL attributes for target object type"
    - "Generate comprehensive attribute list with data types and constraints from XSD"
    - "Document XSD attribute relationships and mutual exclusivity rules"
    - "Create XSD attribute→implementation mapping specifications"
    - "Validate XSD attribute completeness against Snowflake documentation"
    
  STATE_ATTRIBUTE_DISCOVERY:
    - "Execute SHOW/DESCRIBE commands to identify state attributes NOT in XSD"
    - "Query INFORMATION_SCHEMA tables for operational metadata properties"
    - "Document system-generated properties (timestamps, owners, states, counters)"
    - "Identify computed/derived attributes not captured in XSD"
    - "Map state attributes to appropriate INFORMATION_SCHEMA columns"
    
  ATTRIBUTE_CATEGORIZATION:
    - "Categorize ALL attributes: XSD_CONFIGURATION vs STATE_OPERATIONAL vs COMPUTED"
    - "Map XSD attributes to INFORMATION_SCHEMA columns where available"
    - "Document attributes requiring SHOW commands vs INFORMATION_SCHEMA queries"
    - "Identify attributes with special handling requirements (NULL vs 'null' vs default)"
    - "Create complete attribute→SQL_source mapping matrix"

VALIDATION_CHECKPOINT_1.1:
  - [ ] XSD parsed completely with ALL attributes extracted for object type
  - [ ] XSD attribute list generated with types, constraints, and relationships
  - [ ] State attributes identified through SHOW/DESCRIBE command execution
  - [ ] Complete attribute categorization matrix created (XSD vs STATE vs COMPUTED)
  - [ ] XSD→INFORMATION_SCHEMA mapping validated for all applicable attributes
  - [ ] SQL source strategy documented for each attribute category
```

### Step 2: XSD Compliance Validation and Implementation Strategy
```yaml
BLOCKING_VALIDATION_1.2:
  TYPE: "XSD_COMPLIANCE_IMPLEMENTATION_STRATEGY_COMPLETE"  
  REQUIREMENT: "XSD compliance validation framework defined with complete implementation strategy for 100% attribute coverage"
  DELIVERABLE: "XSD compliance testing specifications + implementation strategy + automated validation framework design"
  FAILURE_ACTION: "STOP - Must have XSD compliance strategy before proceeding to requirements documentation"

RESEARCH_ACTIVITIES:
  XSD_COMPLIANCE_FRAMEWORK:
    - "Design automated XSD attribute extraction utilities for snapshot generators"
    - "Specify XSD compliance testing framework that validates 100% attribute coverage"
    - "Create XSD attribute→snapshot generator mapping validation rules"
    - "Document XSD compliance failure detection and reporting mechanisms"
    - "Define XSD evolution handling strategy for future attribute additions"
    
  IMPLEMENTATION_STRATEGY_DESIGN:
    - "Specify snapshot generator implementation patterns for XSD attributes"
    - "Design INFORMATION_SCHEMA query strategies for XSD attribute mapping"
    - "Document SHOW command integration for attributes not in INFORMATION_SCHEMA"
    - "Create attribute parsing and type conversion specifications"
    - "Design error handling for missing or invalid attribute values"
    
  TESTING_FRAMEWORK_SPECIFICATION:
    - "Specify unit testing patterns for XSD compliance validation"
    - "Design integration testing framework for complete attribute capture"
    - "Create test harness patterns for snapshot validation with XSD attributes"
    - "Document automated regression testing for XSD compliance maintenance"
    - "Specify performance testing strategy for complete attribute snapshot operations"

VALIDATION_CHECKPOINT_1.2:
  - [ ] XSD compliance validation framework specifications complete
  - [ ] Automated XSD attribute extraction utility design documented
  - [ ] Snapshot generator implementation strategy defined for XSD attributes
  - [ ] INFORMATION_SCHEMA + SHOW command integration strategy specified
  - [ ] Testing framework design complete for XSD compliance validation
  - [ ] Error handling and edge case management strategy documented
```
