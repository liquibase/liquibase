...
AIPL classifies all steps into five categories for consistent execution patterns:

| Category | Purpose | Pattern | Required Sections |
|----------|---------|---------|-------------------|
| **RESEARCH_STEP** | Gather information from external sources | Input sources → Search commands → Validation checks | INPUT_REQUIREMENTS, EXECUTION_STEPS, ACCEPTANCE_CRITERIA, DELIVERABLES |
| **PROCESSING_STEP** | Transform/analyze gathered information | Input data → Processing logic → Output structure | INPUT_REQUIREMENTS, EXECUTION_STEPS, ACCEPTANCE_CRITERIA, DELIVERABLES |
| **CREATION_STEP** | Generate artifacts (documents, code, files) | Templates → Generation logic → Quality gates | INPUT_REQUIREMENTS, EXECUTION_STEPS, ACCEPTANCE_CRITERIA, QUALITY_GATE, DELIVERABLES |
| **VALIDATION_STEP** | Verify quality/completeness before proceeding | Quality checks → Pass/fail decisions → Blocking behavior | INPUT_REQUIREMENTS, QUALITY_GATE, DELIVERABLES |
| **EXECUTION_STEP** | Run commands or perform operations | Commands → Success criteria → Error handling | INPUT_REQUIREMENTS, EXECUTION_STEPS, ACCEPTANCE_CRITERIA, DELIVERABLES |

### Universal Step Template

Every step should follow this complete specification pattern:
...
STEP_NAME:
  ...
  QUALITY_GATE:           # For critical steps only
...
AIPL uses specific keywords that trigger consistent AI behaviors:

### Core Keywords

| Keyword | Definition | AI Behavior | Usage |
|---------|------------|-------------|-------|
| **ACCEPTANCE_CRITERIA** | Binary pass/fail conditions that must be met to proceed | Triggers validation behavior - must pass to proceed | Step completion validation |
| **QUALITY_GATE** | Blocking checkpoint that halts execution on failure | Triggers blocking behavior - execution stops on failure | Critical validation points |
| **SUCCESS_CRITERIA** | Measurable conditions defining completion | Triggers completion behavior - defines what constitutes success | Phase completion definition |
| **SPECIFICATIONS** | Detailed implementable requirements with validation commands | Triggers implementation behavior - detailed enough for coding | Requirements documents |
| **DELIVERABLE** | Tangible output artifact with defined structure | Triggers output verification behavior - validates tangible results | Output validation |
| **EXECUTION_STEPS** | Sequenced actions with specific commands | Triggers sequential execution behavior - runs commands in order | Step-by-step procedures |
...
ACCEPTANCE_CRITERIA:
  - CRITERION: "All tests pass"
    VALIDATION_COMMAND: "mvn test -q"
    PASS_CONDITION: "Exit code 0"

# QUALITY_GATE - Blocks execution on failure
QUALITY_GATE:
  NAME: "validate-compilation"
  TYPE: "BLOCKING"
  FAILURE_ACTION: "STOP"
  ERROR_MESSAGE: "Compilation failed - fix errors before proceeding"

# SUCCESS_CRITERIA - Defines completion
SUCCESS_CRITERIA: "FileFormat object fully implemented with working tests"

# DELIVERABLES - Defines expected outputs
DELIVERABLES:
  - ARTIFACT: "FileFormat.java"
    LOCATION: "src/main/java/liquibase/database/object/"
