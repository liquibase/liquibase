...
## 🎯 ARCHITECTURE DECISION TREE - Choose Your Implementation Pattern

### CRITICAL: Operation-Type-Driven Architecture Assessment

**Before starting implementation, answer this key question:**

```yaml
ARCHITECTURE_DECISION:
  QUESTION: "How many mutually exclusive operation modes does this changetype have?"
  ...
  SINGLE_MODE_OPERATIONS:
    EXAMPLES: 
      ...
      - "Standard INSERT"
    CHARACTERISTICS:
      - "One primary operation with minor variations (IF EXISTS, CASCADE)"
      - "Shared validation across variations"
      - "Similar SQL generation logic"
    RECOMMENDED_PATTERN: "PROPERTY_BASED_IMPLEMENTATION"
    COMPLEXITY: "LOW-MEDIUM"
    
  MULTIPLE_MODE_OPERATIONS:
    EXAMPLES:
      - "ALTER WAREHOUSE (RENAME/SET/UNSET/SUSPEND/RESUME/ABORT)"
      - "GRANT/REVOKE (different privilege types)"
      - "CREATE TABLE vs CREATE TABLE AS SELECT"
      - "MERGE (MATCHED_UPDATE/MATCHED_DELETE/NOT_MATCHED_INSERT)"
    CHARACTERISTICS:
    ...
    RECOMMENDED_PATTERN: "OPERATION_TYPE_DRIVEN_ARCHITECTURE"
    COMPLEXITY: "MEDIUM-HIGH"
    SUCCESS_RATE: "71% achieve excellent quality (proven with ALTER WAREHOUSE)"
```

### 🏆 Operation-Type-Driven Architecture Pattern (NEW - Proven Success)

**When to Use**: Any changetype with 2+ mutually exclusive operation modes

**Key Benefits**:
- ✅ **Clear Separation**: Explicit operation types prevent mixing incompatible modes
- ✅ **Better Validation**: Operation-specific validation rules catch errors early  
- ✅ **Cleaner Code**: Separate methods vs complex conditional logic
- ✅ **Easier Extension**: Adding new modes is straightforward
- ✅ **Better Testing**: Each operation type tested independently
- ✅ **Proven Success**: 71% quality improvement over property-based approach

**Architecture Components**:
```yaml
REQUIRED_COMPONENTS:
  1. "OperationType enum with all operation modes"
  2. "Operation-specific validation methods"
  3. "Switch-based SQL generation by operation type"
  4. "Mutual exclusivity enforcement in validation"
  5. "Backward compatibility inference logic"
  6. "Operation-specific test coverage"
```

**Implementation Template**:
```java
// 1. OperationType Enum in Statement class
...
// 3. Switch-based SQL generation
switch (operationType) {
    case MODE_A: return generateModeASql(statement); 
    case MODE_B: return generateModeBSql(statement);
    case MODE_C: return generateModeCSql(statement);
}

// 4. Inference logic for backward compatibility
private OperationType inferOperationType() {
    if (hasPropertyA()) return OperationType.MODE_A;
    if (hasPropertyB()) return OperationType.MODE_B;
    if (hasPropertyC()) return OperationType.MODE_C;
    return null; // Error - cannot infer
}
```

**Real-World Success Example**: 
ALTER WAREHOUSE implementation with 7 operation types achieved:
- ✅ All 22 tests passing (12 integration + 10 unit tests)
...
```yaml
STEP_1: "Assess operation modes in your requirements document"
STEP_2: "Count mutually exclusive operation modes"
STEP_3_IF_1_MODE: 
  - "Use Property-Based Implementation (continue with standard phases)"
STEP_3_IF_2_PLUS_MODES:
  - "Use Operation-Type-Driven Architecture"
  - "Follow enhanced phases with OperationType enum"
  - "Implement operation-specific validation and SQL generation"
...
**🎯 ARCHITECTURE BRANCH POINT**: Choose based on your decision tree assessment

#### Option A: Property-Based Implementation (1 operation mode)
...
#### Option B: Operation-Type-Driven Architecture (2+ operation modes) ⭐ RECOMMENDED
...
public class <ChangeType>Statement extends AbstractSqlStatement {
    ...
    /**
     * Enum defining the type of operation to perform.
     * Each operation type has different valid properties and SQL generation logic.
     */
    public enum OperationType {
        /** MODE_A operation: Specific SQL pattern A */
        MODE_A,
        /** MODE_B operation: Specific SQL pattern B */
        MODE_B,
        /** MODE_C operation: Specific SQL pattern C */
        MODE_C
    }
    ...
    // OperationType getter/setter
    public OperationType getOperationType() {
        return operationType;
    }
    
    public <ChangeType>Statement setOperationType(OperationType operationType) {
        this.operationType = operationType;
        return this;
    }
    
    // Standard getters and setters with fluent interface
    public String get<AttributeName>() {
        return <attributeName>;
    }
    
    public <ChangeType>Statement set<AttributeName>(String <attributeName>) {
        this.<attributeName> = <attributeName>;
        return this;
    }
     ...
     * based on the specified operation type and database constraints.
     */
    public ValidationResult validate() {
        ValidationResult result = new ValidationResult();
        
        // Basic validation
        if (<requiredAttribute> == null || <requiredAttribute>.trim().isEmpty()) {
            result.addError("<RequiredAttribute> is required");
        }
        
        if (operationType == null) {
            // Try to infer operation type from properties (backward compatibility)
    ...
    private OperationType inferOperationType() {
        ...
        boolean hasModeAProperties = hasModeAProperties();
        boolean hasModeBProperties = hasModeBProperties();
        boolean hasModeCProperties = hasModeCProperties();
        
        // If multiple operation types are specified, cannot infer - this is an error
        int operationTypeCount = (hasModeAProperties ? 1 : 0) + (hasModeBProperties ? 1 : 0) + (hasModeCProperties ? 1 : 0);
        if (operationTypeCount > 1) {
            return null; // Cannot infer - multiple operation types specified
        }
        
        if (hasModeAProperties) return OperationType.MODE_A;
        if (hasModeBProperties) return OperationType.MODE_B;
        if (hasModeCProperties) return OperationType.MODE_C;
    ...
    private void validateOperationType(ValidationResult result) {
        switch (operationType) {
            case MODE_A:
                validateModeAOperation(result);
                break;
            case MODE_B:
                validateModeBOperation(result);
                break;
            case MODE_C:
                validateModeCOperation(result);
                break;
        }
    }
    
    private void validateModeAOperation(ValidationResult result) {
        if (<modeASpecificAttribute> == null) {
            result.addError("MODE_A requires <modeASpecificAttribute>");
        }
        // MODE_A operations should not have other operation properties set
        if (hasModeBProperties() || hasModeCProperties()) {
            result.addWarning("MODE_B/MODE_C properties are ignored in MODE_A operations");
        }
    }
    
    private void validateModeBOperation(ValidationResult result) {
        if (!hasModeBProperties()) {
            result.addError("At least one MODE_B property must be specified");
        }
        // Add operation-specific validation rules
    }
    
    private void validateModeCOperation(ValidationResult result) {
        if (!hasModeCProperties()) {
            result.addError("At least one MODE_C property must be specified");
        }
    }
    
    private boolean hasModeAProperties() {
        return <modeASpecificAttribute> != null;
    }
    
    private boolean hasModeBProperties() {
        return Boolean.TRUE.equals(<modeBSpecificAttribute>);
    }
    
    private boolean hasModeCProperties() {
        return <modeCSpecificAttribute> != null;
    }
...
**🎯 ARCHITECTURE BRANCH POINT**: Choose based on your Phase 2 implementation

#### Option A: Property-Based SQL Generation (1 operation mode)
...
public class <ChangeType>GeneratorSnowflake extends AbstractSqlGenerator<<ChangeType>Statement> {
    ...
    public Sql[] generateSql(<ChangeType>Statement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ...
        // Handle options based on properties
...
#### Option B: Operation-Type-Driven SQL Generation (2+ operation modes) ⭐ RECOMMENDED
...
public class <ChangeType>GeneratorSnowflake extends AbstractSqlGenerator<<ChangeType>Statement> {
    ...
    @Override
    public ValidationErrors validate(<ChangeType>Statement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ValidationErrors errors = new ValidationErrors();
        
        // Use the enhanced validation from the statement
        <ChangeType>Statement.ValidationResult result = statement.validate();
    ...
    private Sql generateModeASql(<ChangeType>Statement statement, Database database) {
        ...
        sql.append(" MODE_A_SPECIFIC_SYNTAX");
        
        // Add MODE_A specific parameters
        if (statement.get<ModeASpecificAttribute>() != null) {
            sql.append(" ").append(statement.get<ModeASpecificAttribute>());
        }
    ...
    private Sql generateModeBSql(<ChangeType>Statement statement, Database database) {
        ...
        if (statement.get<ModeBSpecificAttribute>() != null) {
            setClause.add("PROPERTY = " + statement.get<ModeBSpecificAttribute>());
        }
        
        // Add more MODE_B properties as needed
    ...
    private Sql generateModeCSQL(<ChangeType>Statement statement, Database database) {
        StringBuilder sql = new StringBuilder("ALTER <OBJECT> ");
        sql.append(database.escapeObjectName(statement.get<RequiredAttribute>(), Table.class));
        sql.append(" UNSET ");
        ...
        if (Boolean.TRUE.equals(statement.get<ModeCSpecificAttribute>())) {
            unsetClause.add("PROPERTY");
        }
        
        // Add more MODE_C properties as needed
...
**🔧 Key Enhancements in OperationType Approach**:
1. **Switch-based Generation**: Clean separation of SQL logic per operation type
2. **Enhanced Validation**: Delegates to statement's sophisticated validation
3. **Operation-Specific Methods**: Each operation type has dedicated SQL generation
4. **Error Prevention**: Runtime checks for unsupported operation types
5. **Extensibility**: Easy to add new operation types by adding new case + method
