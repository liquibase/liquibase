...
public class LiquibaseLauncher {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        for (File libDirFile : libDirs) {
            ...
            for (File lib : files) {
                if (lib.getName().toLowerCase(Locale.US).endsWith(".jar") && !lib.getName().toLowerCase(Locale.US).equals("liquibase-standard.jar")) {
                    try {
        ...
        //The system classloader's parent is the boot classloader, which keeps the only classloader with liquibase-standard.jar the same as the rest of the classes it needs to access.
