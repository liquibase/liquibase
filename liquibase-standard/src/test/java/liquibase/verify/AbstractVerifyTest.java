...
public class AbstractVerifyTest {
    ...
    protected static class TestState {
        ...
        public TestState(String testName, String testGroup, String stateName, Type type) {
            ...
            if (liquibaseRootDir.getAbsolutePath().endsWith("liquibase-standard")) { //sometimes running in liquibase-standard, especially in maven
                liquibaseRootDir = liquibaseRootDir.getAbsoluteFile().getParentFile();
            }

            this.savedStateDir = new File(liquibaseRootDir.getAbsoluteFile(), "liquibase-standard/src/test/resources/verify/saved_state/"+testName+"/"+testGroup);
        ...
        public void test() throws Exception {
            ...
            if (StringUtils.isEmpty(existingContent) && (StringUtils.trimToNull(stateContent.toString()) != null)) {
                save();
            } else {
                try {
