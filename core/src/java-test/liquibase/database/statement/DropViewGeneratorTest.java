...
public class DropViewGeneratorTest extends AbstractSqStatementTest {
    ...
    protected void setupDatabase(Database database) throws Exception {
        ...
        new Executor(database).execute(new CreateViewStatement(null, VIEW_NAME, "select * from "+TABLE_NAME, false));

        if (database.supportsSchemas()) {
            new Executor(database).execute(new CreateViewStatement(TestContext.ALT_SCHEMA, VIEW_NAME, "select * from "+ TestContext.ALT_SCHEMA+"."+TABLE_NAME, false));
        }
    }
