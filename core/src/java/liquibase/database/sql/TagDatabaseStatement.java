    ...
    public String getSqlStatement(Database database) throws StatementNotSupportedOnDatabaseException {
        ...
        if (database instanceof MySQLDatabase) {
            try {
                ...
                if (version < 5) {
                    return new RawSqlStatement("UPDATE "+database.escapeTableName(database.getDefaultSchemaName(), database.getDatabaseChangeLogTableName())+" C LEFT JOIN (SELECT MAX(DATEEXECUTED) as MAXDATE FROM (SELECT DATEEXECUTED FROM `DATABASECHANGELOG`) AS X) D ON C.DATEEXECUTED = D.MAXDATE SET C.TAG = '"+tag+"' WHERE D.MAXDATE IS NOT NULL").getSqlStatement(database);
                }
