...
public class SchemesCDIConfigBuilder {
    ...
    public CDILiquibaseConfig createCDILiquibaseConfig() {
        ...
        log.debug(String.format("[id = %s] createConfig(). Date: '%s'", id, new Date()));
    ...
    private CDILiquibaseConfig createCDILiquibaseConfig(final String id, final InputStream is) throws IOException {
        ...
        if (output.exists()) {
            log.debug(String.format("[id = %s] File [path='%s'] already exists, deleting", id, path));
            if (output.delete()) {
                log.debug(String.format("[id = %s] File [path='%s'] already exists, deleted successfully.", id, path));
            } else {
                log.debug(String.format("[id = %s] File [path='%s'] already exists, failed to delete.", id, path));
            }
        }
        ...
        log.debug(String.format("[id = %s] Root liquibase file [path='%s'] ready.", id, path));
        ...
        log.debug(String.format("[id = %s] Resolved schemes: %n%s%n", id, schemes));
        log.debug(String.format("[id = %s] Generating root liquibase file...", id));
        ...
        log.debug(String.format("[id = %s] Generation complete.", id));
        log.debug(String.format("[id = %s] Root liquibase xml: %n %s %n", id, xml));
        ...
        )
        {
            while (null == lock) {
                try {
                    ...
                    log.debug(String.format("[id = %s] Lock already acquired, waiting for the lock...", id));
                }
                if (null == lock) {
                    log.debug(String.format("[id = %s] Waiting for the lock...", id));
    ...
    private String copyToFile(final String id, final String liquibase, final String schema) {
        ...
        try {
            ...
            log.debug(String.format("[id = %s] LiquibaseSchema path is [path='%s'].", id, path));
            ...
            log.debug(String.format("[id = %s] LiquibaseSchema file [path='%s'] is ready, copying data...", id, file.getAbsolutePath()));
