...
public class SchemesCDIConfigBuilder {
    ...
    private static final Logger log = LogFactory.getLog(SchemesCDIConfigBuilder.class);
    ...
    public CDILiquibaseConfig createCDILiquibaseConfig() {
        ...
        log.debug(LogTarget.LOG, String.format("[id = %s] createConfig(). Date: '%s'", id, new Date()));
        ...
        try {
            ...
            log.warn(LogTarget.LOG, String.format("[id = %s] Unable to initialize liquibase where '%s'.", id, ex.getMessage()), ex);
            return null;
        } finally {
            try {
                is.close();
            } catch (IOException ioe) {
                log.warn(LogTarget.LOG, String.format("[id = %s] IOException during closing an input stream '%s'.", id, ioe.getMessage()), ioe);
            }
        }
    }
    ...
    private CDILiquibaseConfig createCDILiquibaseConfig(final String id, final InputStream is) throws IOException {
        ...
        log.debug(LogTarget.LOG, String.format("[id = %s] Includes directory: [path='%s']", id, liquibaseDir.getAbsolutePath()));

        String path = String.format("%s/%s", ROOT_PATH, TEMPLATE_NAME);
        File output = new File(path);

        if (output.exists()) {
            log.debug(LogTarget.LOG, String.format("[id = %s] File [path='%s'] already exists, deleting", id, path));
            if (output.delete()) {
                log.debug(LogTarget.LOG, String.format("[id = %s] File [path='%s'] already exists, deleted successfully.", id, path));
            } else {
                log.debug(LogTarget.LOG, String.format("[id = %s] File [path='%s'] already exists, failed to delete.", id, path));
            }
        }
        if (!output.createNewFile()) {
            throw new RuntimeException(String.format("[id = %s] Cannot create [%s] file.", id, output));
        }
        log.info(LogTarget.LOG, String.format("[id = %s] File %s was created.", id, output));
        log.debug(LogTarget.LOG, String.format("[id = %s] Root liquibase file [path='%s'] ready.", id, path));

        long start = System.currentTimeMillis();
        log.info(LogTarget.LOG, String.format("[id = %s] Scanning application for liquibase schemes.", id));
        ...
        log.info(LogTarget.LOG, String.format("[id = %s] Scan complete [took=%s milliseconds].", id, System.currentTimeMillis() - start));
        log.debug(LogTarget.LOG, String.format("[id = %s] Resolved schemes: %n%s%n", id, schemes));
        log.debug(LogTarget.LOG, String.format("[id = %s] Generating root liquibase file...", id));
        ...
        log.info(LogTarget.LOG, String.format("[id = %s] File %s was written.", id, output));
        log.debug(LogTarget.LOG, String.format("[id = %s] Generation complete.", id));
        log.debug(LogTarget.LOG, String.format("[id = %s] Root liquibase xml: %n %s %n", id, xml));
    ...
    CDILiquibaseConfig fileLocked(final String id, Callable<CDILiquibaseConfig> action) throws Exception {
        log.info(LogTarget.LOG, String.format("[id = %s] JVM lock acquired, acquiring file lock", id));
        String lockPath = String.format("%s/schema.liquibase.lock", ROOT_PATH);

        File lockFile = new File(lockPath);
        if (!lockFile.exists() && lockFile.createNewFile()) {
            log.info(LogTarget.LOG, String.format("[id = %s] Created lock file [path='%s'].", id, lockPath));
        }

        log.info(LogTarget.LOG, String.format("[id = %s] Trying to acquire the file lock [file='%s']...", id, lockPath));
        ...
        )
        {
            while (null == lock) {
                try {
                    ...
                    log.debug(LogTarget.LOG, String.format("[id = %s] Lock already acquired, waiting for the lock...", id));
                }
                if (null == lock) {
                    log.debug(LogTarget.LOG, String.format("[id = %s] Waiting for the lock...", id));
                    Thread.sleep(FILE_LOCK_TIMEOUT);
                }
            }
            log.info(LogTarget.LOG, String.format("[id = %s] File lock acquired, running liquibase...", id));
            actionResult = action.call();
            lock.release();
        } catch (Exception e) {
            log.warn(LogTarget.LOG, e.getMessage(), e);
        }
        return actionResult;
    }

    private String copyToFile(final String id, final String liquibase, final String schema) {
        log.info(LogTarget.LOG, String.format("[id = %s] copyToFile(%s, %s)", id, liquibase, schema));

        InputStream is = null;
        try {
            is = this.getClass().getClassLoader().getResourceAsStream(schema);

            log.info(LogTarget.LOG, String.format("[id = %s] Transferring schema [resource=%s] to directory [path=%s]...", id, schema, liquibase));
            String path = schema.startsWith("/") ? schema.substring(1) : schema;
            log.debug(LogTarget.LOG, String.format("[id = %s] LiquibaseSchema path is [path='%s'].", id, path));

            if (path.contains("/")) {

                String dirPath = String.format("%s/%s", liquibase, path.substring(0, path.lastIndexOf('/')));
                log.debug(LogTarget.LOG, String.format("[id = %s] LiquibaseSchema path contains intermediate directories [path='%s'], preparing its...", id, dirPath));

                File file = new File(dirPath);
                if (!file.exists() && file.mkdirs()) {
                    log.info(LogTarget.LOG, String.format("[id = %s] Directories for [path='%s'] file created.", id, file.getAbsolutePath()));
                }
            }

            File file = new File(String.format("%s/%s", liquibase, path));
            if (file.exists()) {
                log.info(LogTarget.LOG, String.format("[id = %s] LiquibaseSchema file [path='%s'] already exists, deleting...", id, file.getAbsolutePath()));
                if (file.delete()) {
                    log.info(LogTarget.LOG, String.format("[id = %s] File [path='%s'] deleted.", id, file.getAbsolutePath()));
                }
            }
            if (file.createNewFile()) {
                log.info(LogTarget.LOG, String.format("[id = %s] File [path='%s'] created.", id, file.getAbsolutePath()));
            }
            log.debug(LogTarget.LOG, String.format("[id = %s] LiquibaseSchema file [path='%s'] is ready, copying data...", id, file.getAbsolutePath()));

            FileUtil.write(StreamUtil.getStreamContents(is), file);

            String schemaPath = file.getAbsolutePath().replace(ROOT_PATH, "");
            log.info(LogTarget.LOG, String.format("[id = %s] Data copied, schema path is [path='%s'].", id, schemaPath));
            return schemaPath;
        } catch (IOException e) {
            ...
            try {
                ...
                log.warn(LogTarget.LOG, String.format("IOException during closing an input stream '%s'.", ioe.getMessage()), ioe);
            }
        }
    }
