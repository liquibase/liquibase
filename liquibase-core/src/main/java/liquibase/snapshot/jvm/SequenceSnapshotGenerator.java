    ...
    protected String getSelectSequenceSql(Schema schema, Database database) {
        if (database instanceof DB2Database) {
            ...
            return "SELECT relname AS SEQUENCE_NAME FROM pg_class, pg_namespace " +
                ...
                return "SELECT SEQUENCE_NAME, MINIMUM_VALUE AS MIN_VALUE, MAXIMUM_VALUE AS MAX_VALUE, INCREMENT AS INCREMENT_BY FROM INFORMATION_SCHEMA.SEQUENCES WHERE SEQUENCE_SCHEMA = '" + schema.getName() +"'";
        } else {
