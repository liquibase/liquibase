...
public abstract class AbstractJdbcDatabase implements Database {
  ...
  public String getAutoIncrementClause(final BigInteger startWith, final BigInteger incrementBy) {
    ...
    final boolean generateStartWith = generateAutoIncrementStartWith(startWith);
    final boolean generateIncrementBy = generateAutoIncrementBy(incrementBy);
  ...
  public String getLiquibaseCatalogName() {
    ...
    final ConfigurationProperty configuration = LiquibaseConfiguration.getInstance().getProperty(GlobalConfiguration.class, GlobalConfiguration.LIQUIBASE_CATALOG_NAME);
  ...
  public String getLiquibaseSchemaName() {
    ...
    final ConfigurationProperty configuration = LiquibaseConfiguration.getInstance().getProperty(GlobalConfiguration.class, GlobalConfiguration.LIQUIBASE_SCHEMA_NAME);
    if (configuration.getWasOverridden()) {
      return configuration.getValue(String.class);
    }
  ...
  public void dropDatabaseObjects(final CatalogAndSchema schemaToDrop) throws LiquibaseException {
    final ObjectQuotingStrategy currentStrategy = this.getObjectQuotingStrategy();
    ...
    try {
      ...
      final DiffResult diffResult = DiffGeneratorFactory.getInstance().compare(new EmptyDatabaseSnapshot(this), snapshot, new CompareControl(snapshot.getSnapshotControl().getTypesToInclude()));
      final List<ChangeSet> changeSets = new DiffToChangeLog(diffResult, new DiffOutputControl(true, true, false).addIncludedSchema(schemaToDrop)).generateChangeSets();
      ...
      final boolean previousAutoCommit = this.getAutoCommitMode();
      ...
      try {
        for (final ChangeSet changeSet : changeSets) {
          ...
          for (final Change change : changeSet.getChanges()) {
            ...
            final SqlStatement[] sqlStatements = change.generateStatements(this);
            for (final SqlStatement statement : sqlStatements) {
  ...
  public boolean isLiquibaseObject(final DatabaseObject object) {
    if (object instanceof Table) {
      final Schema liquibaseSchema = new Schema(getLiquibaseCatalogName(), getLiquibaseSchemaName());
  ...
  public String getViewDefinition(CatalogAndSchema schema, final String viewName) throws DatabaseException {
    schema = schema.customize(this);
    final String definition = ExecutorService.getInstance().getExecutor(this).queryForObject(new GetViewDefinitionStatement(schema.getCatalogName(), schema.getSchemaName(), viewName), String.class);
    if (definition == null) {
      return null;
    }
  ...
  public boolean equals(final Object o) {
    ...
    final AbstractJdbcDatabase that = (AbstractJdbcDatabase) o;
  ...
  public void executeStatements(final Change change, final DatabaseChangeLog changeLog, final List<SqlVisitor> sqlVisitors) throws LiquibaseException {
    final SqlStatement[] statements = change.generateStatements(this);
  ...
  public void saveStatements(final Change change, final List<SqlVisitor> sqlVisitors, final Writer writer) throws IOException, StatementNotSupportedOnDatabaseException, LiquibaseException {
    final SqlStatement[] statements = change.generateStatements(this);
    for (final SqlStatement statement : statements) {
      for (final Sql sql : SqlGeneratorFactory.getInstance().generateSql(statement, this)) {
  ...
  protected List<SqlVisitor> filterRollbackVisitors(final List<SqlVisitor> visitors) {
    ...
    if (visitors != null) {
      for (final SqlVisitor visitor : visitors) {
  ...
  public boolean isFunction(final String string) {
    ...
    for (final DatabaseFunction function : getDateFunctions()) {
