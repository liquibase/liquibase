...
public class InsertExecutablePreparedStatement extends ExecutablePreparedStatementBase {
	...
	protected String generateSql(List<ColumnConfig> cols) {
	    ...
	    sql.append(database.escapeObjectName(new ObjectReference(getCatalogName(), getSchemaName(), getTableName())));
	    sql.append("(");
	    for(ColumnConfig column : getColumns()) {
	        ...
	        sql.append(database.escapeObjectName(column.getName(), Column.class));
	        sql.append(", ");
	        params.append("?, ");
	        cols.add(column);
	    }
