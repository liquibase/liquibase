...
public class TagDatabaseGenerator extends AbstractSqlGenerator<TagDatabaseStatement>
{
    ...
    public ValidationErrors validate(
    		TagDatabaseStatement tagDatabaseStatement,
    		Database database,
    		...
    		SqlGeneratorChain sqlGeneratorChain)
    {
        ...
        if (database instanceof MySQLDatabase) {
                    ...
                    return new Sql[]
            		{
                        new UnparsedSql(
            					...
            					database.escapeTableName(
                    				...
                    				database.getDatabaseChangeLogTableName()) + 
                				" C LEFT JOIN (SELECT MAX(DATEEXECUTED) as MAXDATE" + 
                				"FROM (SELECT DATEEXECUTED FROM `DATABASECHANGELOG`) AS X) D" + 
            ...
            updateStatement.setWhereClause(
            		"DATEEXECUTED = (SELECT MAX(DATEEXECUTED)" + 
    				"FROM (SELECT DATEEXECUTED FROM " + 
    				database.escapeTableName(
    						database.getLiquibaseCatalogName(),
    						liquibaseSchema,
    						database.getDatabaseChangeLogTableName()) + 
					") AS X) AND TAG IS NULL");
        } 
        else if (database instanceof InformixDatabase) {
            return new Sql[] {
                    new UnparsedSql("SELECT MAX(dateexecuted) max_date FROM " +
                			database.escapeTableName(database.getLiquibaseCatalogName(),
                					liquibaseSchema,
                					database.getDatabaseChangeLogTableName()) +
            ...
            updateStatement.setWhereClause(
            		"DATEEXECUTED = (SELECT MAX(DATEEXECUTED) FROM " +
    				database.escapeTableName(
						database.getLiquibaseCatalogName(),
						liquibaseSchema,
						database.getDatabaseChangeLogTableName()) +
