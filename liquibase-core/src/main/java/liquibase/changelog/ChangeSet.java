...
public class ChangeSet implements Conditional, LiquibaseSerializable {
     ...
     */
    private List<ActionVisitor> actionVisitors = new ArrayList<ActionVisitor>();
    ...
    public CheckSum generateCheckSum() {
        ...
        for (ActionVisitor visitor : this.getActionVisitors()) {
        ...
        if (child.getName().equals("rollback")) {
            ...
            for (ParsedNode node : potentialVisitors) {
                ...
                if (actionVisitor != null) {
                    actionVisitor.setApplyToRollback(applyToRollback);
                    if (dbms.size() > 0) {
            ...
            if (!skipChange) {
                ...
                for (Change change : getChanges()) {
                    if ((!(change instanceof DbmsTargetedChange)) || DatabaseList.definitionMatches(((DbmsTargetedChange) change).getDbms(), database, true)) {
                        ...
                        database.executeStatements(change, databaseChangeLog, actionVisitors);
        ...
        try {
            ...
            if (rollBackChanges != null && rollBackChanges.size() > 0) {
                ...
                for (int i = changes.size() - 1; i >= 0; i--) {
                    ...
                    database.executeRollbackStatements(change, actionVisitors);
                }
            }
    ...
    public void addSqlVisitor(ActionVisitor actionVisitor) {
        actionVisitors.add(actionVisitor);
    }
