 ...
 */
@DatabaseChange(name="dropFulltextConstraint", description = "Drops an existing unique constraint", priority = ChangeMetaData.PRIORITY_DEFAULT, appliesTo = "uniqueConstraint")
public class DropFulltextConstraintChange extends AbstractChange {
...
//        }
        DropFulltextConstraintStatement statement = new DropFulltextConstraintStatement(getCatalogName(), getSchemaName(), getTableName(), getConstraintName());
    	if (database instanceof SybaseASADatabase) {
    		statement.setFulltextColumns(uniqueColumns);
    	}
    ...
    public ChangeStatus checkStatus(Database database) {
        try {
            FulltextConstraint example = new FulltextConstraint(getConstraintName(), getCatalogName(), getSchemaName(), getTableName());
            if (getFulltextColumns() != null) {
                for (String column : getFulltextColumns().split("\\s*,\\s*")) {
                    example.addColumn(example.getColumns().size(), column);
                }
            }
            return new ChangeStatus().assertComplete(!SnapshotGeneratorFactory.getInstance().has(example, database), "Fulltext constraint exists");
        } catch (Exception e) {
...
//    				column.getConstraints().setFulltext(false);
//    			}
//				return true;
//			}
...
//    }
    ...
    public String getConfirmationMessage() {
        return "Fulltext constraint "+getConstraintName()+" dropped from "+getTableName();
    }
