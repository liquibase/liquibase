 ...
 */
@DatabaseChange(name="addFulltextConstraint", description = "Adds a unique constrant to an existing column or set of columns.", priority = ChangeMetaData.PRIORITY_DEFAULT, appliesTo = "column")
public class AddFulltextConstraintChange extends AbstractChange {
...
//        }
    	...
    	AddFulltextConstraintStatement statement = new AddFulltextConstraintStatement(getCatalogName(), getSchemaName(), getTableName(), getColumnNames(), getConstraintName());
    ...
    public ChangeStatus checkStatus(Database database) {
        ...
        try {
            FulltextConstraint example = new FulltextConstraint(getConstraintName(), getCatalogName(), getSchemaName(), getTableName(), getColumnNames().split("\\s+,\\s+"));

            FulltextConstraint snapshot = SnapshotGeneratorFactory.getInstance().createSnapshot(example, database);
            result.assertComplete(snapshot != null, "Fulltext constraint does not exist");
...
//    }
    ...
    public String getConfirmationMessage() {
        return "Fulltext constraint added to "+getTableName()+"("+getColumnNames()+")";
    }

    @Override
    protected Change[] createInverses() {
        DropFulltextConstraintChange inverse = new DropFulltextConstraintChange();
