...
public class LiquibaseEntityResolver implements EntityResolver2 {
   ...
   public InputSource resolveEntity(String name, String publicId, String baseURI, String systemId) throws SAXException, IOException {
       log.debug(LogTarget.LOG, "Resolving XML entity name='" + name + "', publicId='" + publicId + "', baseURI='" + baseURI + "', systemId='" + systemId + "'");

       if(systemId == null){
           log.debug(LogTarget.LOG, "Unable to resolve XML entity locally. Will load from network.");
           return null;
       }
    ...
    private InputSource tryResolveFromResourceAccessor(String systemId) {
        ...
        log.debug(LogTarget.LOG, "Attempting to load "+systemId+" from resourceAccessor as "+path);

        try {
            InputStream resourceAsStream = StreamUtil.singleInputStream(path, resourceAccessor);
            if (resourceAsStream == null) {
                log.debug(LogTarget.LOG, "Could not load "+systemId+" from resourceAccessor as "+path);
                return null;
            }
    ...
    public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
        log.warn(LogTarget.LOG, "Current XML parsers seems to not support EntityResolver2. External entities won't be correctly loaded");
