...
public class TableExistsPrecondition implements Precondition {
    ...
    public void check(Database database, DatabaseChangeLog changeLog, ChangeSet changeSet) throws PreconditionFailedException, PreconditionErrorException {
    	try {
            if (!DatabaseObjectGeneratorFactory.getInstance().getGenerator(Table.class, database).has(database.correctSchema(new Schema(getCatalogName(), getSchemaName())), getTableName(), database)) {
