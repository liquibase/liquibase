    ...
    public void testAutoIncrementH2Database() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof H2Database) {
    			...
    			assertEquals("CREATE TABLE CATALOG_NAME.SCHEMA_NAME.TABLE_NAME (COLUMN1_NAME BIGINT AUTO_INCREMENT)", generatedSql[0].toSql());
    		}
    	}
    }
    ...
    public void testAutoIncrementStartWithH2Database() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof H2Database) {
    			...
    			assertEquals("CREATE TABLE CATALOG_NAME.SCHEMA_NAME.TABLE_NAME (COLUMN1_NAME BIGINT AUTO_INCREMENT (START WITH 0))", generatedSql[0].toSql());
    		}
    	}
    }
    ...
    public void testAutoIncrementStartWithIncrementByH2Database() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof H2Database) {
				...
				assertEquals("CREATE TABLE CATALOG_NAME.SCHEMA_NAME.TABLE_NAME (COLUMN1_NAME BIGINT AUTO_INCREMENT (START WITH 0, INCREMENT BY 10))", generatedSql[0].toSql());
    		}
    	}
    }    
