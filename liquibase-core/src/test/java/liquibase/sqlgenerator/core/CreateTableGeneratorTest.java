        ...
        for (Database database : TestContext.getInstance().getAllDatabases()) {
            if (database instanceof OracleDatabase) {
                ...
                if (shouldBeImplementation(database)) {
                    assertEquals("CREATE TABLE CATALOG_NAME.TABLE_NAME (COLUMN1_NAME TIMESTAMP DEFAULT null)", this.generatorUnderTest.generateSql(statement, new ExecutionOptions(new RuntimeEnvironment(database)), null)[0].toSql());
                }
            }
        }
    }
    ...
    public void testAutoIncrementDB2Database() throws Exception {
    	for (Database database : TestContext.getInstance().getAllDatabases()) {
    		if (database instanceof DB2Database) {
	    		...
	    		Sql[] generatedSql = this.generatorUnderTest.generateSql(statement, new ExecutionOptions(new RuntimeEnvironment(database)), null);
