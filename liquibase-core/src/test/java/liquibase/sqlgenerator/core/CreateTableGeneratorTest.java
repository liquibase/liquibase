        ...
        for (Database database : TestContext.getInstance().getAllDatabases()) {
            if (database instanceof OracleDatabase) {
                ...
                if (shouldBeImplementation(database)) {
                    assertEquals("CREATE TABLE CATALOG_NAME.TABLE_NAME (COLUMN1_NAME TIMESTAMP DEFAULT null)", this.generatorUnderTest.generateActions(statement, new ExecutionEnvironment(database), null)[0].describe());
                }
            }
        }
    }
...
//    }
    ...
    public void createReferencesSchemaEscaped() throws Exception {
        ...
        CreateTableStatement statement = new CreateTableStatement(CATALOG_NAME, SCHEMA_NAME, TABLE_NAME);
        statement.addConstraint(new ForeignKeyConstraint("fk_test_parent", TABLE_NAME + "(id)").addColumns("id"));
        Action[] generatedSql = this.generatorUnderTest.generateActions(statement, new ExecutionEnvironment(database), null);
