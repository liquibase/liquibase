...
public class DatabaseFactoryTest {
    ...
    public void openConnectionThrowsExceptionWhenDriverCannotBeFoundByUrl() throws Exception {
        DatabaseException thrown = assertThrows(DatabaseException.class, () -> databaseFactory.openConnection("not:a:driver", "", "", null, resourceAccessor));
        assertThat(thrown.getCause(), instanceOf(RuntimeException.class));
        assertThat(thrown.getMessage(), containsString("Driver class was not specified and could not be determined from the url"));
    }
    ...
    public void openConnectionThrowsRuntimeExceptionWhenDriverPropertiesFileNotFound() throws Exception {
        DatabaseException thrown = assertThrows(DatabaseException.class, () -> databaseFactory.openConnection("jdbc:h2:mem:DatabaseFactoryTest", "sa", "", null, null, "unknown file", null, resourceAccessor));
        assertThat(thrown.getCause(), instanceOf(RuntimeException.class));
        assertThat(thrown.getMessage(), containsString("Can't open JDBC Driver specific properties from the file"));
    }
