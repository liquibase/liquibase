...
public abstract class AbstractAntTaskTest {
    ...
    protected static void setProperties() {
        ...
        try {
            absoluteFilePath = URLDecoder.decode(absoluteFilePath, GlobalConfiguration.OUTPUT_ENCODING.getCurrentValue());
        ...
        try {
            testAbsoluteFilePath = URLDecoder.decode(testAbsoluteFilePath, GlobalConfiguration.OUTPUT_ENCODING.getCurrentValue());
        } catch (UnsupportedEncodingException e) {
            throw new RuntimeException("Missing UTF-8 encoding in JVM.", e);
        }
