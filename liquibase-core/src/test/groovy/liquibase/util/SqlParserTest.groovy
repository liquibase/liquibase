...
class SqlParserTest extends Specification {
    ...
    def "parse with standard options"() {
        ...
        "select * from dual\ngo\ninsert into test\ngo"                                                     | [["select", "*", "from", "dual"] , ["insert", "into", "test"]]
    }

    @Unroll("#featureName `#input`")
    def "parse with whitespace preserved"() {
        expect:
        SqlParser.parse(input, true, true, false, true)*.toArray(true) == output
    ...
    def "parse with comments preserved"() {
        expect:
        SqlParser.parse(input, true, false, true, true)*.toArray(true) == output
