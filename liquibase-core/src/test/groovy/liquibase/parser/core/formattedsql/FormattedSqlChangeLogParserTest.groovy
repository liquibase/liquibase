...
public class FormattedSqlChangeLogParserTest extends Specification {
  ...
  private static final String COLUMN_EXISTS_CHANGELOG =
          "--liquibase formatted sql\n" +
          "\n" +
          // Changeset for testing column-exists precondition
          "--changeset talanb:preconditions_column_exists\n" +
          "--preconditions onFail:MARK_RAN onerror:HALT\n" +
          "--precondition-column-exists schema:my_schema table:my_table column:my_column\n" +
          "alter table my_table drop column my_column;\n" +
            // Changeset for testing column-exists precondition
          "--changeset talanb:preconditions_not_column_exists\n" +
          "--preconditions onFail:MARK_RAN onerror:HALT\n" +
          "--precondition-not-column-exists table:my_table schema:my_schema column:my_column\n" +
          "alter table my_table add (id int);\n"
  

  private static final String TABLE_EXISTS_CHANGELOG =
          "--liquibase formatted sql" +
                  "\n" +
                  // Changeset for testing table-exists precondition
                  "--changeset talanb:preconditions_table_exists\n" +
                  "--preconditions onFail:MARK_RAN onerror:HALT\n" +
                  "--precondition-table-exists schema:my_schema table:my_table\n" +
                  "drop table my_table;\n" +
                  // Changeset for testing table-exists precondition
                  "--changeset talanb:preconditions_not_table_exists\n" +
                  "--preconditions onFail:MARK_RAN onerror:HALT\n" +
                  "--precondition-not-table-exists schema:my_schema table:my_table\n" +
                  "create table my_table (\n" +
                  "  id int primary key\n" +
                  ");\n"

  private static final String PRECONDITION_VARIABLE_REPLACEMENT_CHANGELOG =
          "--liquibase formatted sql\n" +
          "\n" +
          "--changeset talanb:precondition_with_variable\n" +
          "--preconditions onFail:MARK_RAN onerror:HALT\n" +
          "--precondition-table-exists schema:\${schemaname} table:my_table\n" +
          "drop table my_table;\n"
  ...
  def "precondition variable replacement"() throws Exception {
    expect:
    ChangeLogParameters params = new ChangeLogParameters()
    params.set("schemaname", "my_schema")
    DatabaseChangeLog changeLog = new MockFormattedSqlChangeLogParser(PRECONDITION_VARIABLE_REPLACEMENT_CHANGELOG).parse(
            "asdf.sql",
            params,
            new JUnitResourceAccessor())
    changeLog.getChangeSets().size() == 1

    changeLog.getChangeSets().get(0).getAuthor() == "talanb"
    changeLog.getChangeSets().get(0).getId() == "precondition_with_variable"
    changeLog.getChangeSets().get(0).getPreconditions() != null
    changeLog.getChangeSets().get(0).getPreconditions().getNestedPreconditions().size() == 1
    changeLog.getChangeSets().
            get(0).
            getPreconditions().
            getNestedPreconditions().
            get(0) instanceof TableExistsPrecondition
    ((TableExistsPrecondition) changeLog.getChangeSets().
            get(0).
            getPreconditions().
            getNestedPreconditions().
            get(0)).tableName == "my_table"
    ((TableExistsPrecondition) changeLog.getChangeSets().
            get(0).
            getPreconditions().
            getNestedPreconditions().
            get(0)).schemaName == "my_schema"
    changeLog.getChangeSets().get(0).getChanges().size() == 1
    changeLog.getChangeSets().get(0).getChanges().get(0) instanceof RawSQLChange
    ((RawSQLChange) changeLog.getChangeSets().get(0).getChanges().get(0)).getSql() == "drop table my_table;"

  }

  def "column exists precondition parsing"() throws Exception {
    expect:
    ChangeLogParameters params = new ChangeLogParameters()
    params.set("schemaname", "schema1")
    DatabaseChangeLog changeLog = new MockFormattedSqlChangeLogParser(COLUMN_EXISTS_CHANGELOG).parse(
            "asdf.sql",
            params,
            new JUnitResourceAccessor())
    ...
    changeLog.getChangeSets().get(0).getAuthor() == "talanb"
    changeLog.getChangeSets().get(0).getId() == "preconditions_column_exists"
    changeLog.getChangeSets().get(0).getPreconditions() != null
    changeLog.getChangeSets().get(0).getPreconditions().getNestedPreconditions().size() == 1
    changeLog.getChangeSets().
            get(0).
            getPreconditions().
            getNestedPreconditions().
            get(0) instanceof ColumnExistsPrecondition
    ((ColumnExistsPrecondition) changeLog.getChangeSets().
            get(0).
            getPreconditions().
            getNestedPreconditions().
            get(0)).tableName == "my_table"
    ...
    ((ColumnExistsPrecondition) changeLog.getChangeSets().
            ...
            getPreconditions().
            getNestedPreconditions().
            get(0)).columnName == "my_column"
    changeLog.getChangeSets().get(0).getChanges().size() == 1
    changeLog.getChangeSets().get(0).getChanges().get(0) instanceof RawSQLChange
    ((RawSQLChange) changeLog.getChangeSets().
            get(0).
            getChanges().
            get(0)).getSql() == "alter table my_table drop column my_column;"
    
    changeLog.getChangeSets().get(1).getAuthor() == "talanb"
    changeLog.getChangeSets().get(1).getId() == "preconditions_not_column_exists"
    changeLog.getChangeSets().get(1).getPreconditions() != null
    changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().size() == 1
    changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().get(0) instanceof NotPrecondition
    ((NotPrecondition) changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().get(0)).
            getNestedPreconditions().
            get(0) instanceof ColumnExistsPrecondition
    ((ColumnExistsPrecondition) ((NotPrecondition) changeLog.getChangeSets().
            get(1).
            getPreconditions().
            getNestedPreconditions().
            get(0)).getNestedPreconditions().get(0)).tableName == "my_table"
    ((ColumnExistsPrecondition) ((NotPrecondition) changeLog.getChangeSets().
            get(1).
            getPreconditions().
            getNestedPreconditions().
            get(0)).getNestedPreconditions().get(0)).schemaName == "my_schema"
    ((ColumnExistsPrecondition) ((NotPrecondition) changeLog.getChangeSets().
            get(1).
            getPreconditions().
            getNestedPreconditions().
            get(0)).getNestedPreconditions().get(0)).columnName == "my_column"
    changeLog.getChangeSets().get(1).getChanges().size() == 1
    changeLog.getChangeSets().get(1).getChanges().get(0) instanceof RawSQLChange
    ((RawSQLChange) changeLog.getChangeSets().
            get(1).
            getChanges().
            get(0)).getSql() == "alter table my_table add (id int);"
    
  }
  
  def "table exists precondition parsing"() throws Exception {
    expect:
    ChangeLogParameters params = new ChangeLogParameters()
    params.set("schemaname", "schema1")
    DatabaseChangeLog changeLog = new MockFormattedSqlChangeLogParser(TABLE_EXISTS_CHANGELOG).parse(
            "asdf.sql",
            params,
            new JUnitResourceAccessor())
    changeLog.getChangeSets().size() == 2
    changeLog.getChangeSets().get(0).getAuthor() == "talanb"
    changeLog.getChangeSets().get(0).getId() == "preconditions_table_exists"
    changeLog.getChangeSets().get(0).getPreconditions() != null
    changeLog.getChangeSets().get(0).getPreconditions().getNestedPreconditions().size() == 1
    changeLog.getChangeSets().
            get(0).
    ...
    changeLog.getChangeSets().get(1).getAuthor() == "talanb"
    changeLog.getChangeSets().get(1).getId() == "preconditions_not_table_exists"
    changeLog.getChangeSets().get(1).getPreconditions() != null
    changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().size() == 1
    changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().get(0) instanceof NotPrecondition
    ((NotPrecondition) changeLog.getChangeSets().get(1).getPreconditions().getNestedPreconditions().get(0)).
            getNestedPreconditions().
            get(0) instanceof TableExistsPrecondition
    ((TableExistsPrecondition) ((NotPrecondition) changeLog.getChangeSets().
            get(1).
            getPreconditions().
            getNestedPreconditions().
            get(0)).getNestedPreconditions().get(0)).tableName == "my_table"
    ...
    changeLog.getChangeSets().get(1).getChanges().size() == 1
    changeLog.getChangeSets().get(1).getChanges().get(0) instanceof RawSQLChange
    ((RawSQLChange) changeLog.getChangeSets().get(1).getChanges().get(0)).getSql() == "create table my_table (\n" +
            "  id int primary key\n" +
            ");"
  ...
  def parse() throws Exception {
    ...
    DatabaseChangeLog changeLog = new MockFormattedSqlChangeLogParser(VALID_CHANGELOG).parse(
            "asdf.sql",
            params,
            new JUnitResourceAccessor())
