    ...
    def "Can apply single column with standard settings but complex names"() {
        ...
        [conn, scope, snapshot, columnName, table] << JUnitScope.instance.getSingleton(ConnectionSupplierFactory).connectionSuppliers.collectMany {
            ...
            return CollectionUtil.permutations([
                    ...
                    [scope],
                    [snapshot],
                    new TestColumnSupplier().getObjectNames(Column, scope).collect {it.name+"_2"},
                    snapshot.get(Table)
            ])
        }
    ...
    def "Can apply multiple columns with standard settings but complex names"() {
        ...
        [conn, scope, snapshot, columnNames, table] << JUnitScope.instance.getSingleton(ConnectionSupplierFactory).connectionSuppliers.collectMany {
            ...
            return CollectionUtil.permutations([
                    ...
                    [scope],
                    [snapshot],
                    new TestColumnSupplier().getObjectNames(Column, scope).collect {[it.name+"_2", it.name+"_3"]},
                    snapshot.get(Table)
            ])
        }
    }
...
//        columnDef.constraints;
        ...
        where:
        [conn, scope, snapshot, columnName, table, dataType, defaultValue, remarks, primaryKey, isNullable, autoIncrementInformation] << JUnitScope.instance.getSingleton(ConnectionSupplierFactory).connectionSuppliers.collectMany {
